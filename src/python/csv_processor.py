...
logger = structlog.get_logger("csv_processor")
...
class CSVProcessor:
    ...
    async def read_csv_content(self, file: UploadFile) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.error("File encoding error", 
                             filename=file.filename, 
                             error=str(e))
            ...
            self.logger.error("Failed to read CSV", 
                             filename=file.filename, 
                             error=str(e))
            ...
            try:
                if expected_type == 'datetime':
                    ...
                    if validated_df[column].isna().any():
                        ...
                        self.logger.warning("Invalid numeric values found",
                                          column=column,
                ...
                self.logger.error("Data type validation failed",
                                 column=column,
                                 expected_type=expected_type,
                                 error=str(e))
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid data type in column '{column}'. Expected: {expected_type}"
                )
    ...
    def process_payments_csv(self, company_id: int, df: pd.DataFrame, db: Session) -> List[Payment]:
        ...
        self.logger.info("Processing payments CSV", 
        ...
        if not db_ops.companies.company_exists(db, company_id):
        ...
        for index, row in df.iterrows():
            try:
                ...
                existing_payments = db_ops.payments.get_customer_payments(
                    db, company_id, str(row['customer_id'])
                )
                ...
                self.logger.warning("Failed to process payment row",
                                   row_index=index + 1,
                                   error=str(e),
                                   row_data=row.to_dict())
        
        if errors:
            self.logger.error("Errors occurred during CSV processing",
                             error_count=len(errors),
                             errors=errors[:5])  # Log first 5 errors
            raise HTTPException(
                status_code=400,
                detail=f"Failed to process {len(errors)} rows. First error: {errors[0]}"
            )
        
        self.logger.info("Payments processing completed",
                        company_id=company_id,
...
class PaymentsCSVProcessor(CSVProcessor):
    ...
    async def process_file(self, company_id: int, file: UploadFile, db: Session) -> Dict[str, Any]:
        ...
        self.logger.info("Starting payments CSV processing",
                        company_id=company_id,
                        filename=file.filename)
        ...
        try:
            db.commit()
            self.logger.info("Payments CSV processing completed successfully",
                           company_id=company_id,
                           filename=file.filename,
