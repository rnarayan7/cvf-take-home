...
from schemas import (
    ...
    PaymentResponse,
    ThresholdCreate, ThresholdResponse,
    MetricsResponse
)
from csv_processor import get_payments_csv_processor
from db_operations import get_db_operations, DatabaseOperations
...
async def create_cohort(
    ...
    logger.info("Creating cohort", company_id=company_id, cohort_month=cohort.cohort_month, planned_sm=cohort.planned_sm)
    ...
    try:
        return db_ops.cohorts.create_cohort(
            company_id, cohort.cohort_month, cohort.planned_sm,
            cohort.sharing_percentage, cohort.cash_cap
        )
    except Exception as e:
...
async def upload_payments(
    ...
    logger.info("Payment upload completed", 
...
async def create_threshold(
    ...
    return db_ops.thresholds.create_threshold(company_id, threshold.payment_period_month, 
                                             threshold.minimum_payment_percent)
...
async def get_cohorts_table(
    ...
    try:
        ...
        logger.info("Cohorts table generated", 
                   company_id=company_id,
                   cohort_count=len(rows),
                   period_count=len(columns))
...
async def get_cashflows(
    ...
    try:
        ...
        logger.info("Cashflows generated", 
                   company_id=company_id,
                   cohort_count=len(cashflow_cohorts))
