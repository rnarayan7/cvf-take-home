...
from models import Company, Cohort, Payment, Threshold, CashflowSnapshot, RecalcJob
from database import get_db

logger = structlog.get_logger("db_operations")
...
class CohortOperations:
    ...
    def create_cohort(self, company_id: int, cohort_month: date, planned_sm: float,
                     sharing_percentage: float = 0.5, cash_cap: float = 0.0) -> Cohort:
    ...
    def get_cohort(self, company_id: int, cohort_month: date) -> Optional[Cohort]:
        ...
        return self.db.query(Cohort).filter(
            and_(Cohort.company_id == company_id, Cohort.cohort_month == cohort_month)
        ).first()
...
class PaymentOperations:
    ...
    def create_payment(self, company_id: int, customer_id: str, 
                      payment_date: date, cohort_month: date, amount: float, 
                      commit: bool = False) -> Payment:
    ...
    def get_customer_payments(self, company_id: int, customer_id: str) -> List[Payment]:
        ...
        return self.db.query(Payment).filter(
            and_(Payment.company_id == company_id, Payment.customer_id == customer_id)
        ).all()
...
class ThresholdOperations:
    ...
    def create_threshold(self, company_id: int, payment_period_month: int,
                        minimum_payment_percent: float) -> Threshold:
        """Create a new threshold"""
        logger.info("Creating threshold",
                   company_id=company_id,
...
class RecalcJobOperations:
    ...
    def update_job_status(self, job_id: int, status: str, 
                         log: Optional[str] = None, error_message: Optional[str] = None) -> None:
    ...
    def list_jobs_by_company(self, company_id: int, limit: int = 50) -> List[RecalcJob]:
        ...
        return (self.db.query(RecalcJob)
