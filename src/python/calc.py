...
def payment_df_to_cohort_df(payment_df: pd.DataFrame) -> pd.DataFrame:
    ...
    cohort_matrix = grouped_df.reset_index().pivot(
        index="cohort", 
    ...
    for cohort in result.index:
        ...
        if last_actual_period >= 0:
            ...
            for period in range(last_actual_period + 1, horizon_months):
                ...
                if (period in cohort_df.columns and 
                    pd.notna(cohort_df.at[cohort, period]) and 
                    cohort_df.at[cohort, period] != 0):
...
def apply_threshold_to_cohort_df(
    ...
    threshold_by_period = {
        int(t['payment_period_month']): float(t['minimum_payment_percent']) 
        for t in thresholds
    }
...
def get_cvf_cashflows_df(
    ...
    def normalize_date(date_str: str) -> pd.Timestamp:
        return pd.to_datetime(date_str).to_period('M').dt.to_timestamp()
    ...
    for trade in trades:
        ...
        failures = threshold_mask.loc[cohort, periods] if cohort in threshold_mask.index else pd.Series(False, index=periods)
...
def calculate_current_month_owed(
